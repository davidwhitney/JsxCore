...
public sealed class TypeDefinitionOptions
{
    ...
    /// <summary>
    /// Bare module specifier the generated declarations are imported from.
    /// </summary>
    /// <remarks>
    /// <para>
    /// Defaults to "@jsxcore/generated", which is deliberately explicit: at the import site it is
    /// obvious that the types are produced from .NET rather than hand-written, so nobody edits the
    /// declaration files expecting the change to survive.
    /// </para>
    /// <para>
    /// This is the prefix; a type is imported from its own namespace's module, for example
    /// "@jsxcore/generated/SampleApp/Models". The bare specifier holds only types that have no
    /// .NET namespace at all. Nothing is re-exported from it.
    /// </para>
    /// </remarks>
    public string ModuleSpecifier { get; set; } = "@jsxcore/generated";
    ...
    /// <c>@jsxcore/generated/SampleApp/Models</c>, so two types sharing a simple name in different
    ...
    /// <c>MyApp.Web.ViewModels</c> can be imported as <c>@jsxcore/generated/ViewModels</c> rather
